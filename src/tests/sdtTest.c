...
void assertMemoryValue(int8_t *memory, uint64_t address, int64_t expectedValue, int size, int *failCount) {
    int64_t actualValue = load(memory, address, size);
...
void assertRegisterValue(int64_t *registers, int regNum, int64_t expectedValue, int *failCount) {
    int64_t actualValue = registers[regNum];
    if (actualValue != expectedValue) {
        printf("Assertion failed: Register X%d expected 0x%lx, but got 0x%lx\n", regNum, expectedValue, actualValue);
        (*failCount)++;
    }
}
